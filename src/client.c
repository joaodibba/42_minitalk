...
void encode_and_send_char(pid_t server_pid, int c)
{
    ...
    ft_printf("the num: %i\n", c);
    i = 8; // initialize i to 7 instead of 1
    while (i--) // decrement i instead of incrementing
    {
        if (c & (1 << i))
...
void	send_client_pid(pid_t server_pid, pid_t client_pid)
{
	int	i;
...
int	main(int argc, char **argv)
{
	...
	// Compare server_pid with integer value of argv[1]
